"""RSI Reversal Strategy — Tutorial Backtest.

The most well-known technical strategy: buy when RSI < 30, sell when RSI > 70.
Does this textbook edge actually exist?

Setup: EURUSD, 1h bars, 5 years (2021-04 to 2026-03), Tradeview ILC costs.
"""
...
# ── Load data ──────────────────────────────────────────────────────────
df = pd.read_csv("/tmp/eurusd_1h_5year.csv")
...
print(f"Loaded: {len(df)} bars, {dt_index[0]} → {dt_index[-1]}")
...
# ── Run simulation (Grade A: open prices + costs) ──────────────────────
results = simulate_trades(
    highs, lows, closes,
    signal_bars, directions, sl_distances, tp_distances,
    max_hold=48,  # 48h max hold (2 days)
    exit_mode="rr",
    open_prices=opens,
...
print(f"\n{'='*60}")
print(f"RSI(14) Reversal — EURUSD 1h — 5 Years")
print(f"{'='*60}")
...
for dir_val, name in [(LONG, "LONG (RSI<30)"), (SHORT, "SHORT (RSI>70)")]:
    mask = directions[:len(pnl)] == dir_val  # match trade count
    if len(pnl) < len(directions):
        # Some signals may not have filled
        mask = mask[:len(pnl)]
    dir_pnl = pnl[mask]
    if len(dir_pnl) > 0:
